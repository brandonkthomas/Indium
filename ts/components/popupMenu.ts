export type PopupMenuPlacement = 'right' | 'left' | 'top' | 'bottom';
...
function resolveAnchor(anchor: string | HTMLElement): HTMLElement | null {
...
function getPlacementOrder(preferred: PopupMenuPlacement): PopupMenuPlacement[] {
    const order: PopupMenuPlacement[] = [preferred, 'top', 'bottom', 'left', 'right'];
...
class PopupMenuManager {
    ...
    open(options: PopupMenuOptions): PopupMenuHandle {
        this.activeClose?.();
        ...
        requestAnimationFrame(() => {
            ...
            const firstButton = menu.querySelector<HTMLButtonElement>('.ui-popupmenu-item:not(:disabled)');
            firstButton?.focus();
        });
...
function positionMenu(
    ...
    const fitting = candidates.find((candidate) => (
        candidate.left >= gap
        && candidate.top >= gap
        && candidate.left + menuRect.width <= window.innerWidth - gap
        && candidate.top + menuRect.height <= window.innerHeight - gap
    ));
...
const popupMenuManager = new PopupMenuManager();
