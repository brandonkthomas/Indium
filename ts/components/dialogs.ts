...
import { logEvent, LogData, LogLevel } from '../internal/logging';
...
// Convenience globals are optional and must be explicitly enabled by host config
declare global {
    interface Window {
...
let legacyWindowDialogsEnabled = false;
...
export function configureLegacyWindowDialogs(enabled: boolean) {
    const next = !!enabled;
